...
public class Hoi4BuildingReference extends PsiReferenceBase<Hoi4Key> implements PsiPolyVariantReference {

    private final String name;

    public Hoi4BuildingReference(@NotNull Hoi4Key key) {
            ...
            for (VirtualFile topLevelDir : root.getChildren()) {
                if ("common".equals(topLevelDir.getName())) {
                    for (VirtualFile commonDir : topLevelDir.getChildren()) {
                        if ("buildings".equals(commonDir.getName())) {
    ...
    private List<ResolveResult> findInFile(@NotNull PsiFile containingFile) {
        ...
        return Arrays.stream(children)
                ...
                .filter(sb -> sb.getKey() != null && "buildings".equals(sb.getKey().getText()))
